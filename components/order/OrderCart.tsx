...
export const OrderCart = ({ order }: TOrderCartProps) => {
    return (
        <section
        ...
        >
            ...
            <dl className="mt-6 space-y-4 h-40 overflow-y-auto">
                {order.orderProducts.map(product => (
                    <div key={product.productId} className="flex items-center gap-2 border-t border-gray-200 pt-4">
                        <dt className="flex items-center text-sm text-indigo-800">
                            <span className="font-bold mr-2">({product.quantity}) {''}</span>
                        </dt>
                        <dd className="text-sm font-medium text-indigo-800">{product.product.name}</dd>
                    </div>
                ))}
